...
let AlignmintContract = (_dec = NearBindgen({}), _dec2 = call({
  payableFunction: true
}), _dec3 = view({}), _dec4 = view({}), _dec(_class = (_class2 = class AlignmintContract {
  projects = new UnorderedMap('map-uid-1');
  ...
  createProject({
    ...
    let owner = predecessorAccountId();
    ...
    this.projects.set(owner, JSON.stringify(update)); // Return the total number of projects created so far
...
}, (_applyDecoratedDescriptor(_class2.prototype, "createProject", [_dec2], Object.getOwnPropertyDescriptor(_class2.prototype, "createProject"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "get_projects", [_dec3], Object.getOwnPropertyDescriptor(_class2.prototype, "get_projects"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "number_of_projects", [_dec4], Object.getOwnPropertyDescriptor(_class2.prototype, "number_of_projects"), _class2.prototype)), _class2)) || _class);
function number_of_projects() {
  let _state = AlignmintContract._getState();

  if (!_state && AlignmintContract._requireInit()) {
    throw new Error("Contract must be initialized");
  }

  let _contract = AlignmintContract._create();

  if (_state) {
    AlignmintContract._reconstruct(_contract, _state);
  }

  let _args = AlignmintContract._getArgs();

  let _result = _contract.number_of_projects(_args);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(AlignmintContract._serialize(_result));
}
function get_projects() {
  ...
  let _result = _contract.get_projects(_args);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(AlignmintContract._serialize(_result));
}
function createProject() {
  ...
  let _result = _contract.createProject(_args);

  AlignmintContract._saveToStorage(_contract);

  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(AlignmintContract._serialize(_result));
}

export { createProject, get_projects, number_of_projects };
